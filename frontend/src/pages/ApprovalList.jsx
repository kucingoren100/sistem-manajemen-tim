...
export default function ApprovalList() {
  ...
  // Filter status
  const byStatus = filter === 'all' ? data : data.filter(p => p.status === filter);
  ...
  const filtered = filterByPeriode(byStatus);
  const totalPages = Math.ceil(filtered.length / ITEMS_PER_PAGE);
  const paginated = filtered.slice((page - 1) * ITEMS_PER_PAGE, page * ITEMS_PER_PAGE);

  const handleFilterChange = (val) => { setFilter(val); setPage(1); };
  const handlePeriodeChange = (val) => { setPeriode(val); setPage(1); };
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex flex-wrap justify-between items-center gap-3 mb-6">
        <div>
          <h2 className="text-2xl font-bold text-gray-800">Approval Pengajuan</h2>
          <p className="text-gray-500 text-sm">Tinjau dan proses pengajuan dari anggota tim</p>
        </div>
      ...
      <div className="space-y-3">
        {paginated.length === 0 && (
          <div className="bg-white rounded-xl border p-8 text-center text-gray-400">Tidak ada pengajuan pada periode ini</div>
        )}
        {paginated.map(p => (
        ...
        {totalPages > 1 && (
          <div className="flex justify-between items-center px-4 py-3 border-t bg-white rounded-xl border">
